...
import com.recognition.entity.vo.GoodsVo;
...
public class recognitionPlus01 {
    public static RecognitionResult recognize(int sensorTolerance, List<Layer> openLayer, List<Layer> closeLayer, List<GoodsVo> goodsList, List<Stock> stockList) {
    ...
    public static List<Map<String, Integer>> match(int sensorTolerance,int diff, List<Stock> layerStocks, Map<String, GoodsVo> goodsMap){
        List<Map<String, Integer>> result = new ArrayList<>();
        Map<String,Integer> tempMap=new HashMap<>();
        matchHelper(0,sensorTolerance,diff,layerStocks,goodsMap,0,tempMap,result);

        return result;
    }
    public static void matchHelper(double goodsTolerance, int sensorTolerance,double diff, List<Stock> layerStocks, Map<String, GoodsVo> goodsMap,int start,Map<String,Integer> tempMap,List<Map<String, Integer>> result){

        //波动是[-10,10],goodsTolerance即商品的包装容差
        //本次只是简单的将所有商品的包装容差全部加起来，还有待改善
        if((diff<=sensorTolerance+goodsTolerance)&&(diff>=-1*(sensorTolerance+goodsTolerance))){
            // 目标和达到，添加当前的tempMap到结果中，并返回
            result.add(new HashMap<>(tempMap));
            return;
        }
        for(int i=start;i<layerStocks.size();i++){
            GoodsVo goods=goodsMap.get(layerStocks.get(i).getGoodsId());
            if(diff-goods.getWeight()>=0){
                //尝试添加当前索引
                tempMap.put(goods.getId(),tempMap.getOrDefault(goods.getId(),0)+1);
                goodsTolerance+=goods.getWeight()*(goods.getPackageTolerance()/100);
                //商品数量不能超过层架的数量
                if(tempMap.get(goods.getId())<=layerStocks.get(i).getNum()){
                    matchHelper(goodsTolerance,sensorTolerance,diff-goods.getWeight(),layerStocks,goodsMap,i,tempMap,result);
                }
...
//        close.add(new Layer(2, 1897));//输出为错，不在范围内
        close.add(new Layer(2, 1898));//包装容差是88*0.05=4.4，还有传感器容差10，即1897.6-1906.4是对的
        close.add(new Layer(3, 2000));//实现传感器的容差
        ...
        goodsList.add(new GoodsVo("000001", 60,5));
        goodsList.add(new GoodsVo("000002", 88,5));
        goodsList.add(new GoodsVo("000003", 68,5));
        goodsList.add(new GoodsVo("000004", 123,5));
        goodsList.add(new GoodsVo("000005", 321,5));
        goodsList.add(new GoodsVo("000006", 550,5));
        goodsList.add(new GoodsVo("000007", 126,5));
